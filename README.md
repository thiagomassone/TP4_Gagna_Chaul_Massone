# TP N°4: Bases de datos y manejo de versiones

<p align="center">
  <img width="351" height="173" alt="image" src="https://github.com/user-attachments/assets/acff4f97-07dc-4e86-b9b5-d3abf76122bc" />
</p>

## Curso: Informática Médica - 16.22
## Grupo 1

## Integrantes:
Francisco Gagna - 55224 
Thiago Massone - 60035
Mateo Chaul - 61036

Fecha: 16/05/2026

## Profesores:
MelinaELINA Leonor Piacentino Castaño
Ingrid Celia Spessotti


## Descripción del proyecto

Este repositorio contiene la resolución del TP4 correspondiente al diseño e implementación de una base de datos para un centro médico. Incluye:

- **Parte 1 — Bases de datos**
- **Parte 2 — SQL**
- **Parte 3 — Manejo de versiones con Git y Github** 
